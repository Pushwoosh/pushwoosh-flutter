...
### Updated
- Pushwoosh Android SDK updated to 6.9.0
- Pushwoosh iOS SDK updated to 7.0.45
...
### Breaking Changes
- Removed `sender_id` parameter from `Pushwoosh.initialize()`. The parameter was deprecated and is now fully removed — the Firebase project is determined automatically from `google-services.json`. If your code passes `sender_id` in `initialize()`, simply remove it:
  ```dart
